...
public final class MyFileDao_Impl implements MyFileDao {
    ...
    public List<MyFile> getAll() {
        @SuppressLint("RestrictedApi") RoomSQLiteQuery roomSQLiteQueryAcquire = RoomSQLiteQuery.acquire("SELECT * FROM MyFiles", 0);
        this.__db.assertNotSuspendingTransaction();
    ...
    public MyFile getFileByPathAndName(final String path, final String name) {
        @SuppressLint("RestrictedApi") RoomSQLiteQuery roomSQLiteQueryAcquire = RoomSQLiteQuery.acquire("SELECT * FROM MyFiles WHERE path LIKE ? AND name LIKE ?", 2);
    ...
    public List<MyFile> getNotDownloadedFiles() {
        @SuppressLint("RestrictedApi") RoomSQLiteQuery roomSQLiteQueryAcquire = RoomSQLiteQuery.acquire("SELECT * FROM MyFiles WHERE downloaded = 0", 0);
        this.__db.assertNotSuspendingTransaction();
        Cursor cursorQuery = DBUtil.query(this.__db, roomSQLiteQueryAcquire, false, null);
        try {
            int columnIndexOrThrow = CursorUtil.getColumnIndexOrThrow(cursorQuery, "id");
    ...
    public List<Long> getNotDownloadedFilesSize() {
        @SuppressLint("RestrictedApi") RoomSQLiteQuery roomSQLiteQueryAcquire = RoomSQLiteQuery.acquire("SELECT size FROM MyFiles WHERE downloaded = 0", 0);
        this.__db.assertNotSuspendingTransaction();
        Cursor cursorQuery = DBUtil.query(this.__db, roomSQLiteQueryAcquire, false, null);
        try {
            ArrayList arrayList = new ArrayList(cursorQuery.getCount());
