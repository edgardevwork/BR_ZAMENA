...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    public final PathMeasure f368pm = new PathMeasure();
    ...
    */
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        C3424L.beginSection("StrokeContent#draw");
        if (Utils.hasZeroScaleAxis(matrix)) {
            C3424L.endSection("StrokeContent#draw");
            return;
        }
        ...
        for (int i2 = 0; i2 < this.pathGroups.size(); i2++) {
            ...
            if (pathGroup.trimPath != null) {
                ...
                C3424L.beginSection("StrokeContent#buildPath");
                this.path.reset();
                for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
                    this.path.addPath(((PathContent) pathGroup.paths.get(size)).getPath(), matrix);
                }
                C3424L.endSection("StrokeContent#buildPath");
                C3424L.beginSection("StrokeContent#drawPath");
                canvas.drawPath(this.path, this.paint);
                C3424L.endSection("StrokeContent#drawPath");
            }
        }
        C3424L.endSection("StrokeContent#draw");
    }
    ...
    */
    public final void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        C3424L.beginSection("StrokeContent#applyTrimPath");
        if (pathGroup.trimPath == null) {
            C3424L.endSection("StrokeContent#applyTrimPath");
            return;
        }
        this.path.reset();
        for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
        ...
        this.f368pm.setPath(this.path, false);
        float length = this.f368pm.getLength();
        while (this.f368pm.nextContour()) {
            length += this.f368pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f368pm.setPath(this.trimPathPath, false);
            float length2 = this.f368pm.getLength();
            if (fMin > length) {
    ...
    public void getBounds(RectF rectF, Matrix matrix, boolean z) {
        C3424L.beginSection("StrokeContent#getBounds");
        this.path.reset();
        ...
        C3424L.endSection("StrokeContent#getBounds");
    }

    public final void applyDashPatternIfNeeded(Matrix matrix) {
        C3424L.beginSection("StrokeContent#applyDashPattern");
        if (this.dashPatternAnimations.isEmpty()) {
            C3424L.endSection("StrokeContent#applyDashPattern");
            return;
        }
