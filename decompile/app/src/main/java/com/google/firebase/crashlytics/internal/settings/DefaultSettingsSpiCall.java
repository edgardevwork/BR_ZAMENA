...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            this.logger.m1355d("Requesting settings from " + this.url);
            this.logger.m1361v("Settings query params were: " + queryParamsFor);
            return handleResponse(httpGetRequestApplyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m1358e("Settings request failed.", e);
            return null;
        }
    }
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        this.logger.m1361v("Settings response code was: " + iCode);
        if (requestWasSuccessful(iCode)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        this.logger.m1357e("Settings request failed; (status: " + iCode + ") from " + this.url);
        return null;
    }
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            this.logger.m1364w("Failed to parse settings JSON from " + this.url, e);
            this.logger.m1363w("Settings response " + str);
            return null;
        }
    }
