...
final class ReportQueue {
        ...
        synchronized (this.queue) {
            try {
                ...
                if (z) {
                    ...
                    if (isQueueAvailable()) {
                        Logger.getLogger().m1355d("Enqueueing report: " + crashlyticsReportWithSessionId.getSessionId());
                        Logger.getLogger().m1355d("Queue size: " + this.queue.size());
                        this.singleThreadExecutor.execute(new ReportRunnable(crashlyticsReportWithSessionId, taskCompletionSource));
                        Logger.getLogger().m1355d("Closing task for report: " + crashlyticsReportWithSessionId.getSessionId());
                        taskCompletionSource.trySetResult(crashlyticsReportWithSessionId);
                        return taskCompletionSource;
                    }
                    calcStep();
                    Logger.getLogger().m1355d("Dropping report due to queue being full: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    public void flushScheduledReportsIfAble() {
        ...
        new Thread(new Runnable() { // from class: com.google.firebase.crashlytics.internal.send.ReportQueue$$ExternalSyntheticLambda0
            ...
            public final void run() {
                this.f$0.lambda$flushScheduledReportsIfAble$0(countDownLatch);
            }
        }).start();
    ...
    public void sendReport(final CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, final TaskCompletionSource<CrashlyticsReportWithSessionId> taskCompletionSource) {
        Logger.getLogger().m1355d("Sending report through Google DataTransport: " + crashlyticsReportWithSessionId.getSessionId());
        ...
        this.transport.schedule(Event.ofUrgent(crashlyticsReportWithSessionId.getReport()), new TransportScheduleCallback() { // from class: com.google.firebase.crashlytics.internal.send.ReportQueue$$ExternalSyntheticLambda1
            ...
            public final void onSchedule(Exception exc) {
                this.f$0.lambda$sendReport$1(taskCompletionSource, z, crashlyticsReportWithSessionId, exc);
            }
        });
    }
    ...
    public final class ReportRunnable implements Runnable {
        ...
        public void run() throws InterruptedException {
            ...
            Logger.getLogger().m1355d("Delay for: " + String.format(Locale.US, "%.2f", Double.valueOf(dCalcDelay / 1000.0d)) + " s for report: " + this.reportWithSessionId.getSessionId());
