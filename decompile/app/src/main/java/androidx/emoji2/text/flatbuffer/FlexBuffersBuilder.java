...
public class FlexBuffersBuilder {
    ...
    /* renamed from: bb */
    public final ReadWriteBuf f295bb;
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i) {
        ...
        this.keyComparator = new Comparator<Value>() { // from class: androidx.emoji2.text.flatbuffer.FlexBuffersBuilder.1
            ...
            public int compare(Value value, Value value2) {
                ...
                int i3 = value2.key;
                do {
                    b = FlexBuffersBuilder.this.f295bb.get(i2);
                    b2 = FlexBuffersBuilder.this.f295bb.get(i3);
                    if (b == 0) {
                        return b - b2;
                    }
                    i2++;
    ...
    public final int putKey(String str) {
        ...
        int iWritePosition = this.f295bb.writePosition();
        if ((this.flags & 1) != 0) {
            Integer num = this.keyPool.get(str);
            if (num == null) {
                byte[] bytes = str.getBytes(StandardCharsets.UTF_8);
                this.f295bb.put(bytes, 0, bytes.length);
                this.f295bb.put((byte) 0);
        ...
        this.f295bb.put(bytes2, 0, bytes2.length);
        this.f295bb.put((byte) 0);
        this.keyPool.put(str, Integer.valueOf(iWritePosition));
        return iWritePosition;
    }
    ...
    public final Value writeBlob(int i, byte[] bArr, int i2, boolean z) {
        ...
        int iWritePosition = this.f295bb.writePosition();
        this.f295bb.put(bArr, 0, bArr.length);
        if (z) {
            this.f295bb.put((byte) 0);
        }
    ...
    public final int align(int i) {
        int i2 = 1 << i;
        int iPaddingBytes = Value.paddingBytes(this.f295bb.writePosition(), i2);
        while (true) {
    ...
    public final void writeInt(long j, int i) {
        if (i == 1) {
            this.f295bb.put((byte) j);
            return;
        }
        if (i == 2) {
            this.f295bb.putShort((short) j);
        } else if (i == 4) {
            this.f295bb.putInt((int) j);
        } else {
            if (i != 8) {
                return;
            }
            this.f295bb.putLong(j);
        }
    }
    ...
    public ByteBuffer finish() {
        int iAlign = align(this.stack.get(0).elemWidth(this.f295bb.writePosition(), 0));
        writeAny(this.stack.get(0), iAlign);
        this.f295bb.put(this.stack.get(0).storedPackedType());
        this.f295bb.put((byte) iAlign);
        this.finished = true;
        return ByteBuffer.wrap(this.f295bb.data(), 0, this.f295bb.writePosition());
    }
    ...
    public final Value createVector(int i, int i2, int i3, boolean z, boolean z2, Value value) {
        ...
        if (value != null) {
            iMax = Math.max(iMax, value.elemWidth(this.f295bb.writePosition(), 0));
            i4 = 3;
        } else {
            i4 = 1;
        }
        ...
        for (int i7 = i2; i7 < this.stack.size(); i7++) {
            iMax2 = Math.max(iMax2, this.stack.get(i7).elemWidth(this.f295bb.writePosition(), i7 + i4));
            if (z && i7 == i2) {
        ...
        if (!z) {
            while (i8 < this.stack.size()) {
                this.f295bb.put(this.stack.get(i8).storedPackedType(iMax2));
                i8++;
            }
        }
        if (value != null) {
    ...
    public final void writeOffset(long j, int i) {
        writeInt((int) (this.f295bb.writePosition() - j), i);
    }
    ...
    public final void writeDouble(double d, int i) {
        if (i == 4) {
            this.f295bb.putFloat((float) d);
        } else if (i == 8) {
            this.f295bb.putDouble(d);
        }
    }
    ...
    public final Value createKeyVector(int i, int i2) {
        ...
        while (i3 < this.stack.size()) {
            i3++;
            iMax = Math.max(iMax, Value.elemWidth(4, 0, this.stack.get(i3).key, this.f295bb.writePosition(), i3));
        }
