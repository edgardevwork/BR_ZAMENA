...
public final class PsshAtomUtil {
    ...
    public static byte[] parseSchemeSpecificData(byte[] bArr, UUID uuid) {
        ...
        if (!uuid.equals(psshAtom.uuid)) {
            Log.m635w(TAG, "UUID mismatch. Expected: " + uuid + ", got: " + psshAtom.uuid + ".");
            return null;
        }
    ...
    public static PsshAtom parsePsshAtom(byte[] bArr) {
        ...
        if (i != iBytesLeft) {
            Log.m635w(TAG, "Advertised atom size (" + i + ") does not match buffer size: " + iBytesLeft);
            return null;
        }
        int i2 = parsableByteArray.readInt();
        if (i2 != 1886614376) {
            Log.m635w(TAG, "Atom type is not pssh: " + i2);
            return null;
        }
        int fullAtomVersion = Atom.parseFullAtomVersion(parsableByteArray.readInt());
        if (fullAtomVersion > 1) {
            Log.m635w(TAG, "Unsupported pssh version: " + fullAtomVersion);
            return null;
        }
        ...
        if (unsignedIntToInt2 != iBytesLeft2) {
            Log.m635w(TAG, "Atom data size (" + unsignedIntToInt2 + ") does not match the bytes left: " + iBytesLeft2);
            return null;
        }
