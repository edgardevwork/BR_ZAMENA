...
public final class PesReader implements TsPayloadReader {
    ...
    public final void consume(ParsableByteArray parsableByteArray, int i) throws ParserException {
        ...
        if ((i & 1) != 0) {
            ...
            if (i2 != 0 && i2 != 1) {
                if (i2 == 2) {
                    Log.m635w(TAG, "Unexpected start indicator reading extended header");
                } else if (i2 == 3) {
                    if (this.payloadSize != -1) {
                        Log.m635w(TAG, "Unexpected start indicator: expected " + this.payloadSize + " more bytes");
                    }
    ...
    public final boolean parseHeader() {
        ...
        if (bits != 1) {
            Log.m635w(TAG, "Unexpected start code prefix: " + bits);
            this.payloadSize = -1;
            return false;
        }
        ...
        if (bits2 == 0) {
            ...
            this.payloadSize = i;
            if (i < 0) {
                Log.m635w(TAG, "Found negative packet payload size: " + this.payloadSize);
                this.payloadSize = -1;
            }
        }
        return true;
