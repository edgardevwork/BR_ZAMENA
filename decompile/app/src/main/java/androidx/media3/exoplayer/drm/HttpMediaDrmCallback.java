...
public final class HttpMediaDrmCallback implements MediaDrmCallback {
    ...
    public byte[] executeKeyRequest(UUID uuid, ExoMediaDrm.KeyRequest keyRequest) throws MediaDrmCallbackException {
        ...
        if (TextUtils.isEmpty(licenseServerUrl)) {
            ...
            Uri uri = Uri.EMPTY;
            throw new MediaDrmCallbackException(builder.setUri(uri).build(), uri, ImmutableMap.m1190of(), 0L, new IllegalStateException("No license URL"));
        }
        HashMap map = new HashMap();
        UUID uuid2 = C2732C.PLAYREADY_UUID;
        if (uuid2.equals(uuid)) {
            str = ResourceUriFetcher.MIME_TYPE_XML;
        } else {
            str = C2732C.CLEARKEY_UUID.equals(uuid) ? "application/json" : "application/octet-stream";
        }
