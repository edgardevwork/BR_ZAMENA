...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    public static final float xRotate(float sin, float cos, float cx, float cy, float x, float y) {
        return (((x - cx) * cos) - ((y - cy) * sin)) + cx;
    }

    public static final float yRotate(float sin, float cos, float cx, float cy, float x, float y) {
    ...
    public void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f8 = startTimePoint.f232x;
        float f9 = startTimePoint.f233y;
        float f10 = (endTimePoint.f232x + (f4 / 2.0f)) - ((f5 / 2.0f) + f8);
        float f11 = (endTimePoint.f233y + (f6 / 2.0f)) - (f9 + (f7 / 2.0f));
        float f12 = ((f4 - f5) * f2) / 2.0f;
        this.f232x = (int) ((f8 + (f10 * f)) - f12);
        float f13 = ((f6 - f7) * f3) / 2.0f;
        this.f233y = (int) ((f9 + (f11 * f)) - f13);
        ...
        this.mMode = 0;
        this.f232x = (int) (((startTimePoint.f232x + (f14 * f10)) + (f16 * f11)) - f12);
        this.f233y = (int) (((startTimePoint.f233y + (f10 * f15)) + (f11 * f)) - f13);
    ...
    public void initPolar(int parentWidth, int parentHeight, KeyPosition c, MotionPaths s, MotionPaths e) {
        ...
        if (i == 1) {
            ...
            float f10 = e.f232x;
            float f11 = s.f232x;
            this.f232x = (f9 * (f10 - f11)) + f11;
            if (!Float.isNaN(c.mPercentY)) {
                f2 = c.mPercentY;
            }
            float f12 = e.f233y;
            float f13 = s.f233y;
            this.f233y = (f2 * (f12 - f13)) + f13;
        } else if (i == 2) {
            ...
            float f19 = e.f232x;
            float f20 = s.f232x;
            this.f232x = (f18 * (f19 - f20)) + f20;
            if (!Float.isNaN(c.mPercentY)) {
                f2 = c.mPercentY;
            }
            float f21 = e.f233y;
            float f22 = s.f233y;
            this.f233y = (f2 * (f21 - f22)) + f22;
        }
    ...
    public void setupRelative(MotionController mc, MotionPaths relative) {
        double d = ((this.f232x + (this.width / 2.0f)) - relative.f232x) - (relative.width / 2.0f);
        double d2 = ((this.f233y + (this.height / 2.0f)) - relative.f233y) - (relative.height / 2.0f);
        this.mRelativeToController = mc;
        this.f232x = (float) Math.hypot(d2, d);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f233y = (float) (Math.atan2(d2, d) + 1.5707963267948966d);
        } else {
            this.f233y = (float) Math.toRadians(this.mRelativeAngle);
        }
    }
    ...
    public void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f8 = startTimePoint.f232x;
        float f9 = startTimePoint.f233y;
        float f10 = endTimePoint.f232x + (f4 / 2.0f);
        float f11 = endTimePoint.f233y + (f6 / 2.0f);
        float f12 = (f4 - f5) * f2;
        this.f232x = (int) ((f8 + ((f10 - ((f5 / 2.0f) + f8)) * f)) - (f12 / 2.0f));
        float f13 = (f6 - f7) * f3;
        this.f233y = (int) ((f9 + ((f11 - (f9 + (f7 / 2.0f))) * f)) - (f13 / 2.0f));
        this.width = (int) (f5 + f12);
        this.height = (int) (f7 + f13);
        this.mMode = 2;
        if (!Float.isNaN(c.mPercentX)) {
            this.f232x = (int) (c.mPercentX * ((int) (parentWidth - this.width)));
        }
        if (!Float.isNaN(c.mPercentY)) {
            this.f233y = (int) (c.mPercentY * ((int) (parentHeight - this.height)));
        }
    ...
    public void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f6 = startTimePoint.f232x;
        float f7 = startTimePoint.width;
        float f8 = startTimePoint.f233y;
        float f9 = startTimePoint.height;
        float f10 = (endTimePoint.f232x + (endTimePoint.width / 2.0f)) - ((f7 / 2.0f) + f6);
        float f11 = (endTimePoint.f233y + (endTimePoint.height / 2.0f)) - ((f9 / 2.0f) + f8);
        float f12 = f10 * f;
        float f13 = (f4 * f2) / 2.0f;
        this.f232x = (int) ((f6 + f12) - f13);
        float f14 = f * f11;
        float f15 = (f5 * f3) / 2.0f;
        this.f233y = (int) ((f8 + f14) - f15);
        this.width = (int) (f7 + r7);
        this.height = (int) (f9 + r8);
        float f16 = Float.isNaN(c.mPercentY) ? 0.0f : c.mPercentY;
        this.mMode = 1;
        float f17 = (int) ((startTimePoint.f232x + f12) - f13);
        float f18 = (int) ((startTimePoint.f233y + f14) - f15);
        this.f232x = f17 + ((-f11) * f16);
        this.f233y = f18 + (f10 * f16);
        this.mAnimateRelativeTo = this.mAnimateRelativeTo;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean zDiff = diff(this.f232x, points.f232x);
        boolean zDiff2 = diff(this.f233y, points.f233y);
    ...
    public void getCenter(double p, int[] toUse, double[] data, float[] point, int offset) {
        float fSin = this.f232x;
        float fCos = this.f233y;
        float f = this.width;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] fArr = {this.position, this.f232x, this.f233y, this.width, this.height, this.mPathRotate};
        int i = 0;
        for (int i2 : toUse) {
            if (i2 < 6) {
                data[i] = fArr[r2];
