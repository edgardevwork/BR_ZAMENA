...
public class Debug {
    ...
    public static void printStack(String msg, int n) {
        ...
        for (int i = 1; i <= iMin; i++) {
            StackTraceElement stackTraceElement = stackTrace[i];
            String str2 = ".(" + stackTrace[i].getFileName() + StringUtils.PROCESS_POSTFIX_DELIMITER + stackTrace[i].getLineNumber() + ") ";
            str = str + " ";
    ...
    public static void dumpPoc(Object obj) throws IllegalAccessException, SecurityException, IllegalArgumentException {
        ...
        for (Field field : cls.getFields()) {
            try {
                ...
                if (field.getName().startsWith("layout_constraint") && ((!(obj2 instanceof Integer) || !obj2.toString().equals(ScanBuild.XRAY_FATAL_FAIL_STATUS)) && ((!(obj2 instanceof Integer) || !obj2.toString().equals("0")) && ((!(obj2 instanceof Float) || !obj2.toString().equals("1.0")) && (!(obj2 instanceof Float) || !obj2.toString().equals(YouTubePlayerBridge.RATE_0_5)))))) {
                    System.out.println(str + JsonKt.defaultIndent + field.getName() + " " + obj2);
                }
    ...
    public static String getState(MotionLayout layout, int stateId, int len) throws Resources.NotFoundException {
        ...
        return resourceEntryName.replaceAll(CharBuffer.allocate((resourceEntryName.length() - len) / length).toString().replace((char) 0, '.') + LauncherConstants.UNDERSCORE_STRING, LauncherConstants.UNDERSCORE_STRING);
    }
    ...
    public static void dumpLayoutParams(ViewGroup layout, String str) throws IllegalAccessException, SecurityException, IllegalArgumentException {
        StackTraceElement stackTraceElement = new Throwable().getStackTrace()[1];
        String str2 = ".(" + stackTraceElement.getFileName() + StringUtils.PROCESS_POSTFIX_DELIMITER + stackTraceElement.getLineNumber() + ") " + str + GlideException.IndentedAppendable.INDENT;
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            for (int i2 = 0; i2 < fields.length; i2++) {
                ...
                try {
                    ...
                    if (field.getName().contains("To") && !obj.toString().equals(ScanBuild.XRAY_FATAL_FAIL_STATUS)) {
                        System.out.println(str2 + ConstraintSet.WriteJsonEngine.SPACE + field.getName() + " " + obj);
                    }
                } catch (IllegalAccessException unused) {
                }
            }
    ...
    public static void dumpLayoutParams(ViewGroup.LayoutParams param, String str) throws IllegalAccessException, SecurityException, IllegalArgumentException {
        StackTraceElement stackTraceElement = new Throwable().getStackTrace()[1];
        String str2 = ".(" + stackTraceElement.getFileName() + StringUtils.PROCESS_POSTFIX_DELIMITER + stackTraceElement.getLineNumber() + ") " + str + GlideException.IndentedAppendable.INDENT;
        System.out.println(" >>>>>>>>>>>>>>>>>>. dump " + str2 + GlideException.IndentedAppendable.INDENT + param.getClass().getName());
        for (Field field : param.getClass().getFields()) {
            try {
                Object obj = field.get(param);
                String name = field.getName();
                if (name.contains("To") && !obj.toString().equals(ScanBuild.XRAY_FATAL_FAIL_STATUS)) {
                    System.out.println(str2 + ConstraintSet.WriteJsonEngine.SPACE + name + " " + obj);
                }
            } catch (IllegalAccessException unused) {
            }
        }
