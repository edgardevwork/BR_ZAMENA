...
public class PathInterpolatorCompat implements Interpolator {
    ...
    public final void initPath(Path path) {
        ...
        this.f356mX = new float[iMin];
        this.f357mY = new float[iMin];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < iMin; i2++) {
            pathMeasure.getPosTan((i2 * length) / (iMin - 1), fArr, null);
            this.f356mX[i2] = fArr[0];
            this.f357mY[i2] = fArr[1];
        }
        if (Math.abs(this.f356mX[0]) <= 1.0E-5d && Math.abs(this.f357mY[0]) <= 1.0E-5d) {
            int i3 = iMin - 1;
            if (Math.abs(this.f356mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f357mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < iMin) {
                    float[] fArr2 = this.f356mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(this.f356mX[0]);
        sb.append(",");
        sb.append(this.f357mY[0]);
        sb.append(" end:");
        int i6 = iMin - 1;
        sb.append(this.f356mX[i6]);
        sb.append(",");
        sb.append(this.f357mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f356mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f356mX[i2]) {
        ...
        float[] fArr = this.f356mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f357mY[i];
        }
        float f5 = (f - f3) / f4;
        float[] fArr2 = this.f357mY;
        float f6 = fArr2[i];
