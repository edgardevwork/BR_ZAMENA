...
public final class FloatTweenSpec implements FloatAnimationSpec {
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float targetValue, float initialVelocity) {
        ...
        int i = this.duration;
        return VectorConvertersKt.lerp(initialValue, targetValue, this.easing.transform(RangesKt___RangesKt.coerceIn(i == 0 ? 1.0f : jClampPlayTime / i, 0.0f, 1.0f)));
    }

    private final long clampPlayTime(long playTime) {
        return RangesKt___RangesKt.coerceIn(playTime - this.delay, 0L, this.duration);
    }
