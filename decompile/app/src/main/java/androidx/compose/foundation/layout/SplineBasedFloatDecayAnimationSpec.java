...
public final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    public final double getSplineDeceleration(float velocity) {
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        return initialValue + (flingDistance(initialVelocity) * AndroidFlingSpline.FlingResult.m8027getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m8022flingPositionLfoxSSI(durationNanos > 0 ? playTimeNanos / durationNanos : 1.0f)));
    }
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        long durationNanos = getDurationNanos(0.0f, initialVelocity);
        return ((AndroidFlingSpline.FlingResult.m8028getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m8022flingPositionLfoxSSI(durationNanos > 0 ? playTimeNanos / durationNanos : 1.0f)) * flingDistance(initialVelocity)) / durationNanos) * 1.0E9f;
    }
}
