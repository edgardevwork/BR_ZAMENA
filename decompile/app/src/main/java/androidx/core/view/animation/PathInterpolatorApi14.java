...
public class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    public final float[] f255mX;

    /* renamed from: mY */
    public final float[] f256mY;
    ...
    public PathInterpolatorApi14(Path path) {
        ...
        int i2 = i + 1;
        this.f255mX = new float[i2];
        this.f256mY = new float[i2];
        float[] fArr = new float[2];
        for (int i3 = 0; i3 < i2; i3++) {
            pathMeasure.getPosTan((i3 * length) / i, fArr, null);
            this.f255mX[i3] = fArr[0];
            this.f256mY[i3] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f255mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f255mX[i2]) {
        ...
        float[] fArr = this.f255mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f256mY[i];
        }
        float f5 = (f - f3) / f4;
        float[] fArr2 = this.f256mY;
        float f6 = fArr2[i];
