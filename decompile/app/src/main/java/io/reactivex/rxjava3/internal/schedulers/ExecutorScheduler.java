...
public final class ExecutorScheduler extends Scheduler {
    ...
    public Disposable scheduleDirect(@NonNull Runnable run) {
        ...
        try {
            if (this.executor instanceof ExecutorService) {
                ScheduledDirectTask scheduledDirectTask = new ScheduledDirectTask(runnableOnSchedule, this.interruptibleWorker);
    ...
    public Disposable scheduleDirect(@NonNull Runnable run, final long delay, final TimeUnit unit) {
        ...
        delayedRunnable.timed.replace(SingleHolder.HELPER.scheduleDirect(new DelayedDispose(delayedRunnable), delay, unit));
    ...
    public Disposable schedulePeriodicallyDirect(@NonNull Runnable run, long initialDelay, long period, TimeUnit unit) {
        if (this.executor instanceof ScheduledExecutorService) {
            try {
                ScheduledDirectPeriodicTask scheduledDirectPeriodicTask = new ScheduledDirectPeriodicTask(RxJavaPlugins.onSchedule(run), this.interruptibleWorker);
    ...
    public static final class ExecutorWorker extends Scheduler.Worker implements Runnable {
        ...
        public Disposable schedule(@NonNull Runnable run, long delay, @NonNull TimeUnit unit) {
            ...
            if (executor instanceof ScheduledExecutorService) {
                ...
                scheduledRunnable.setFuture(new DisposeOnCancel(SingleHolder.HELPER.scheduleDirect(scheduledRunnable, delay, unit)));
            }
        ...
        public final class SequentialDispose implements Runnable {
            ...
            public SequentialDispose(SequentialDisposable mar, Runnable decoratedRun) {
                this.mar = mar;
        ...
        public static final class InterruptibleRunnable extends AtomicInteger implements Runnable, Disposable {
            ...
            public InterruptibleRunnable(Runnable run, DisposableContainer tasks) {
                this.run = run;
                this.tasks = tasks;
            }
    ...
    public final class DelayedDispose implements Runnable {
        ...
        /* renamed from: dr */
        public final DelayedRunnable f9765dr;

        public DelayedDispose(DelayedRunnable dr) {
            this.f9765dr = dr;
        }

        @Override // java.lang.Runnable
        public void run() {
            DelayedRunnable delayedRunnable = this.f9765dr;
