...
class KeysMap {
    ...
    public synchronized boolean setKey(String str, String str2) {
        ...
        if (this.keys.size() >= this.maxEntries && !this.keys.containsKey(strSanitizeKey)) {
            Logger.getLogger().m1363w("Ignored entry \"" + str + "\" when adding custom keys. Maximum allowable: " + this.maxEntries);
            return false;
        }
    ...
    public synchronized void setKeys(Map<String, String> map) {
        try {
            ...
            if (i > 0) {
                Logger.getLogger().m1363w("Ignored " + i + " entries when adding custom keys. Maximum allowable: " + this.maxEntries);
            }
        } catch (Throwable th) {
            throw th;
        }
