...
class QueueFileLogStore implements FileLogStore {
    ...
    private LogBytes getLogBytes() {
        ...
        try {
            ...
            Logger.getLogger().m1358e("A problem occurred while reading the Crashlytics log file.", e);
        }
    ...
    private void openLogFile() {
        if (this.logFile == null) {
            try {
                ...
                Logger.getLogger().m1358e("Could not open log file: " + this.workingFile, e);
            }
        }
    }
    ...
    private void doWriteToLog(long j, String str) {
        ...
        try {
            ...
            this.logFile.add(String.format(Locale.US, "%d %s%n", Long.valueOf(j), str.replaceAll("\r", " ").replaceAll(StringUtils.f9903LF, " ")).getBytes(UTF_8));
            while (!this.logFile.isEmpty() && this.logFile.usedBytes() > this.maxLogSize) {
                this.logFile.remove();
            }
        } catch (IOException e) {
            Logger.getLogger().m1358e("There was a problem writing to the Crashlytics log.", e);
        }
    }
}
