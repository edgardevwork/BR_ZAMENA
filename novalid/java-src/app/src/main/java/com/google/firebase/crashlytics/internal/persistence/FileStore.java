...
public class FileStore {
    ...
    private void cleanupFileSystemDir(String str) {
        ...
        if (file.exists() && recursiveDelete(file)) {
            Logger.getLogger().m1355d("Deleted previous Crashlytics file system: " + file.getPath());
        }
    }
    ...
    private static synchronized File prepareBaseDir(File file) {
        try {
            if (file.exists()) {
                ...
                Logger.getLogger().m1355d("Unexpected non-directory file: " + file + "; deleting file and creating new directory.");
                file.delete();
            }
            if (!file.mkdirs()) {
                Logger.getLogger().m1357e("Could not create Crashlytics-specific directory: " + file);
            }
            return file;
        } catch (Throwable th) {
            throw th;
    ...
    public static String sanitizeName(String str) {
        ...
        return str.replaceAll("[^a-zA-Z0-9.]", LauncherConstants.UNDERSCORE_STRING);
    }
}
