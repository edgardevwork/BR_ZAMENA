...
public class RadialViewGroup extends ConstraintLayout {
    ...
    public static final float LEVEL_RADIUS_RATIO = 0.66f;
    ...
    public RadialViewGroup(@NonNull Context context, @Nullable AttributeSet attributeSet, int i) {
        ...
        LayoutInflater.from(context).inflate(C6765R.layout.material_radial_view_group, this);
        ViewCompat.setBackground(this, createBackground());
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C6765R.styleable.RadialViewGroup, i, 0);
        this.radius = typedArrayObtainStyledAttributes.getDimensionPixelSize(C6765R.styleable.RadialViewGroup_materialCircleRadius, 0);
    ...
    public void updateLayoutParams() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (childAt.getId() != C6765R.id.circle_center && !shouldSkipView(childAt)) {
                int i2 = (Integer) childAt.getTag(C6765R.id.material_clock_level);
                if (i2 == null) {
                    i2 = 1;
                }
    ...
    public final void addConstraints(List<View> list, ConstraintSet constraintSet, int i) {
        Iterator<View> it = list.iterator();
        float size = 0.0f;
        while (it.hasNext()) {
            constraintSet.constrainCircle(it.next().getId(), C6765R.id.circle_center, i, size);
