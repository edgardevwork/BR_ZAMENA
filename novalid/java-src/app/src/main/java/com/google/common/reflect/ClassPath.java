...
import com.google.common.p024io.ByteSource;
import com.google.common.p024io.CharSource;
import com.google.common.p024io.Resources;
...
public final class ClassPath {
    ...
    public static final Splitter CLASS_PATH_ATTRIBUTE_SEPARATOR = Splitter.m1152on(" ").omitEmptyStrings();
    ...
    public static class ResourceInfo {
        ...
        /* renamed from: of */
        public static ResourceInfo m1340of(File file, String resourceName, ClassLoader loader) {
        ...
        public final void scanJarFile(JarFile file, ImmutableSet.Builder<ResourceInfo> builder) {
            ...
            while (enumerationEntries.hasMoreElements()) {
                ...
                if (!jarEntryNextElement.isDirectory() && !jarEntryNextElement.getName().equals("META-INF/MANIFEST.MF")) {
                    builder.add((ImmutableSet.Builder<ResourceInfo>) ResourceInfo.m1340of(new File(file.getName()), jarEntryNextElement.getName(), this.classloader));
                }
            }
        }
            ...
            for (File file : fileArrListFiles) {
                ...
                if (file.isDirectory()) {
                    ...
                    if (!str.equals("META-INF/MANIFEST.MF")) {
                        builder.add((ImmutableSet.Builder<ResourceInfo>) ResourceInfo.m1340of(file, str, this.classloader));
                    }
                }
            }
        }
    ...
    @VisibleForTesting
    public static ImmutableSet<File> getClassPathFromManifest(File jarFile, @CheckForNull Manifest manifest) {
        if (manifest == null) {
        ...
        if (value != null) {
            for (String str : CLASS_PATH_ATTRIBUTE_SEPARATOR.split(value)) {
                try {
                    ...
                    if (classPathEntry.getProtocol().equals(PomReader.PomProfileElement.FILE)) {
    ...
    public static ImmutableMap<File, ClassLoader> getClassPathEntries(ClassLoader classloader) {
        ...
        while (it.hasNext()) {
            URL next = it.next();
            if (next.getProtocol().equals(PomReader.PomProfileElement.FILE)) {
    ...
    public static ImmutableList<URL> parseJavaClassPath() {
        ...
        for (String str : Splitter.m1152on(StandardSystemProperty.PATH_SEPARATOR.value()).split(StandardSystemProperty.JAVA_CLASS_PATH.value())) {
            try {
                try {
                    builder.add((ImmutableList.Builder) new File(str).toURI().toURL());
                } catch (SecurityException unused) {
                    builder.add((ImmutableList.Builder) new URL(PomReader.PomProfileElement.FILE, (String) null, new File(str).getAbsolutePath()));
                }
    ...
    public static File toFile(URL url) {
        Preconditions.checkArgument(url.getProtocol().equals(PomReader.PomProfileElement.FILE));
        try {
