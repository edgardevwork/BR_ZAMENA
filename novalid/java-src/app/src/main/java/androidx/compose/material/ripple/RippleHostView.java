...
public final class RippleHostView extends View {
    ...
    public final void m9110addRippleKOepWvA(@NotNull PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, @NotNull Function0<Unit> onInvalidateRipple) {
        ...
        m9111updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            unprojectedRipple.setHotspot(Offset.m11099getXimpl(interaction.getPressPosition()), Offset.m11100getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m9111updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple unprojectedRipple = this.ripple;
        if (unprojectedRipple == null) {
            return;
        }
        unprojectedRipple.trySetRadius(radius);
        unprojectedRipple.m9118setColorDxMtmZc(color, alpha);
        Rect rect = new Rect(0, 0, MathKt__MathJVMKt.roundToInt(Size.m11168getWidthimpl(size)), MathKt__MathJVMKt.roundToInt(Size.m11165getHeightimpl(size)));
        setLeft(rect.left);
        setTop(rect.top);
        setRight(rect.right);
