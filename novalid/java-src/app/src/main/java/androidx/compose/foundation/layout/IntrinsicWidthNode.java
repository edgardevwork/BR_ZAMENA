...
public final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long mo8077calculateContentConstraintsl58MMJ0(@NotNull MeasureScope measureScope, @NotNull Measurable measurable, long j) {
        ...
        if (this.width == IntrinsicSize.Min) {
            iMaxIntrinsicWidth = measurable.minIntrinsicWidth(Constraints.m13621getMaxHeightimpl(j));
        } else {
            iMaxIntrinsicWidth = measurable.maxIntrinsicWidth(Constraints.m13621getMaxHeightimpl(j));
        }
        if (iMaxIntrinsicWidth < 0) {
            iMaxIntrinsicWidth = 0;
        }
        return Constraints.INSTANCE.m13632fixedWidthOenEA2s(iMaxIntrinsicWidth);
    }
