...
public class SimpleArrayMap<K, V> {
    ...
    public final int indexOf(K key, int hash) {
        int i = this.size;
        if (i == 0) {
            return -1;
        }
    ...
    public final <T extends V> T getOrDefaultInternal(Object key, T defaultValue) {
    ...
    public V setValueAt(int i, V value) {
        if (i < 0 || i >= this.size) {
            throw new IllegalArgumentException(("Expected index to be within 0..size()-1, but was " + i).toString());
        }
        int i2 = (i << 1) + 1;
