...
public class MonotonicCurveFit extends CurveFit {
    ...
    /* renamed from: mT */
    public double[] f208mT;
    public double[][] mTangent;

    /* renamed from: mY */
    public double[][] f209mY;
    ...
    public void getPos(double d, double[] dArr) {
        double[] dArr2 = this.f208mT;
        int length = dArr2.length;
        int i = 0;
        int length2 = this.f209mY[0].length;
        if (this.mExtrapolate) {
            double d2 = dArr2[0];
            if (d <= d2) {
                getSlope(d2, this.mSlopeTemp);
                for (int i2 = 0; i2 < length2; i2++) {
                    dArr[i2] = this.f209mY[0][i2] + ((d - this.f208mT[0]) * this.mSlopeTemp[i2]);
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                ...
                while (i < length2) {
                    dArr[i] = this.f209mY[i3][i] + ((d - this.f208mT[i3]) * this.mSlopeTemp[i]);
                    i++;
                }
                return;
            }
        ...
        while (i6 < length - 1) {
            if (d == this.f208mT[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    dArr[i7] = this.f209mY[i6][i7];
                }
            }
            double[] dArr3 = this.f208mT;
            int i8 = i6 + 1;
            double d4 = dArr3[i8];
            if (d < d4) {
                double d5 = dArr3[i6];
    ...
    public void getPos(double d, float[] fArr) {
        ...
        if (this.mExtrapolate) {
            ...
            if (d <= d2) {
                getSlope(d2, this.mSlopeTemp);
                for (int i2 = 0; i2 < length2; i2++) {
                    fArr[i2] = (float) (this.f209mY[0][i2] + ((d - this.f208mT[0]) * this.mSlopeTemp[i2]));
                }
                return;
            }
            int i3 = length - 1;
            ...
            if (d >= d3) {
                getSlope(d3, this.mSlopeTemp);
                while (i < length2) {
                    fArr[i] = (float) (this.f209mY[i3][i] + ((d - this.f208mT[i3]) * this.mSlopeTemp[i]));
                    i++;
                }
                return;
            }
            ...
            if (d <= dArr[0]) {
                for (int i4 = 0; i4 < length2; i4++) {
                    fArr[i4] = (float) this.f209mY[0][i4];
                }
                return;
            }
            int i5 = length - 1;
        ...
        while (i6 < length - 1) {
            if (d == this.f208mT[i6]) {
                for (int i7 = 0; i7 < length2; i7++) {
                    fArr[i7] = (float) this.f209mY[i6][i7];
                }
            }
            double[] dArr2 = this.f208mT;
            int i8 = i6 + 1;
    ...
    public double getPos(double d, int i) {
        ...
        if (d <= dArr[0]) {
            return this.f209mY[0][i];
        }
        int i4 = length - 1;
        if (d >= dArr[i4]) {
            return this.f209mY[i4][i];
        }
        while (i2 < length - 1) {
            double[] dArr2 = this.f208mT;
            double d6 = dArr2[i2];
            if (d == d6) {
                return this.f209mY[i2][i];
            }
            int i5 = i2 + 1;
            double d7 = dArr2[i5];
            if (d < d7) {
    ...
    public static MonotonicCurveFit buildWave(double[] dArr) {
