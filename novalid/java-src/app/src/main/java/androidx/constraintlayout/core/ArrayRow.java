    ...
    */
        ...
        for (int i = 0; i < currentSize; i++) {
            ...
            if (variable != null) {
                ...
                if (variableValue != 0.0f) {
                    ...
                    if (!z) {
                        if (variableValue < 0.0f) {
                            str = str + Constants.LEFT_DELIMITER;
                            variableValue *= -1.0f;
                        }
    ...
    public final SolverVariable pickPivotInVariables(boolean[] zArr, SolverVariable solverVariable) {
        ...
        for (int i = 0; i < currentSize; i++) {
            ...
            if (variableValue < 0.0f) {
                SolverVariable variable = this.variables.getVariable(i);
                if ((zArr == null || !zArr[variable.f191id]) && variable != solverVariable && (((type = variable.mType) == SolverVariable.Type.SLACK || type == SolverVariable.Type.ERROR) && variableValue < f)) {
                    f = variableValue;
                    solverVariable2 = variable;
                }
            }
