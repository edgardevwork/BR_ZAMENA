...
public class SampleQueue implements TrackOutput {
    ...
    public final SpannedData<SharedSampleMetadata> sharedSampleMetadata = new SpannedData<>(new Consumer() { // from class: androidx.media3.exoplayer.source.SampleQueue$$ExternalSyntheticLambda0
        ...
        if (!this.allSamplesAreSyncSamples) {
            ...
            if (i5 == 0) {
                if (!this.loggedUnexpectedNonSyncSample) {
                    Log.m635w(TAG, "Overriding unexpected non-sync sample for format: " + this.upstreamFormat);
