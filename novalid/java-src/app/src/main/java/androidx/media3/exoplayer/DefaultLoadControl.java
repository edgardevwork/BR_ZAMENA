...
public class DefaultLoadControl implements LoadControl {
    ...
    public boolean shouldContinueLoading(long j, long j2, float f) {
        ...
        if (j2 < Math.max(jMin, ExoPlayerImplInternal.PLAYBACK_BUFFER_EMPTY_THRESHOLD_US)) {
            if (!this.prioritizeTimeOverSizeThresholds && z2) {
                z = false;
            }
            this.isLoading = z;
            if (!z && j2 < ExoPlayerImplInternal.PLAYBACK_BUFFER_EMPTY_THRESHOLD_US) {
                Log.m635w("DefaultLoadControl", "Target buffer size reached with less than 500ms of buffered media data.");
            }
